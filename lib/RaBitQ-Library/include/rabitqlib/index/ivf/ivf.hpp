...
class IVF {
   ...
   public:
    ...
    void pure_adc_search(
        const float*, size_t, size_t, const AnnCandidate<float>*, PID*, float*, bool
    ) const;

    void flat_scan(
        const float*, size_t, PID*, float*, bool
    ) const;
...
inline void IVF::pure_adc_search(
    const float* rotated_query,
    size_t k,
    size_t nprobe,
    const AnnCandidate<float>* centroid_dist,
    PID* results,
    float* distances,
    bool use_hacc
) const {
    nprobe = std::min(nprobe, num_cluster_);
    buffer::SearchBuffer<float> knns(k);
    SplitBatchQuery<float> q_obj(
        rotated_query, padded_dim_, ex_bits_, metric_type_, use_hacc
    );
    for (size_t i = 0; i < nprobe; ++i) {
        ...
        if (metric_type_ == METRIC_L2) {
            ...
            auto g_add_ip = dot_product<float>(
                rotated_query, initer_->centroid(cid), padded_dim_
            );
    ...
    if (distances != nullptr) {
        knns.copy_results(results, distances);
    } else {
...
inline void IVF::flat_scan(
    ...
    // Scan ALL clusters with zero G_add (no centroid distances needed)
    for (size_t cid = 0; cid < num_cluster_; ++cid) {
        const Cluster& cur_cluster = cluster_lst_[cid];
        q_obj.set_g_add(0.0f);
        search_cluster(cur_cluster, q_obj, knns, use_hacc);
    }
...
inline void IVF::search(
    ...
    search(query, k, nprobe, results, static_cast<float*>(nullptr), use_hacc);
}
