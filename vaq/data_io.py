"""Dataset I/O for VAQ eval (.bin / .fvecs / .fbin / .bvecs; numpy only)."""
...
def infer_dim_from_path(path: str) -> int:
    if path.endswith(".bin"):
        raise ValueError("Cannot infer dim for .bin files; pass --dim explicitly.")
...
def write_fvecs(filename: str | Path, data: np.ndarray) -> None:
    data = np.asarray(data, dtype=np.float32)
    n, d = data.shape
    dim_array = np.array([d] * n, dtype=np.int32)
    file_out = np.column_stack((dim_array, data.view(np.int32)))
