"""Minimal dataset I/O for RaBitQ eval (numpy + scipy only; no faiss)."""
...
def bin_num_vectors(path: str, dim: int) -> int:
    """Number of float32 vectors in a raw row-major .bin (no header)."""
...
def read_bin_prefix(
    ...
    """
    Read up to max_rows vectors from raw float32 .bin (no header), starting at start_row.
    Uses memmap so large files are not fully loaded into RAM.
    """
...
def read_bvecs_prefix(
    ...
    """SIFT/BIGANN .bvecs: int32 dim + dim uint8 per vector; returns float32."""
...
def infer_dim_from_path(path: str) -> int:
    """Infer vector dimension from file header (not supported for raw .bin)."""
...
def read_fvecs_prefix(
    ...
    """Vectors [start_row : start_row+max_rows) from .fvecs."""
...
def load_vectors_prefix(
    ...
    """Load up to max_rows vectors starting at start_row from .bin / .fvecs / .fbin / .bvecs."""
    if path.endswith(".bin"):
        if dim is None:
