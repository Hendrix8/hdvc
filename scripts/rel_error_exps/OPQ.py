...
# -*- coding: utf-8 -*-
"""
Evaluate Optimized Product Quantization (OPQ) on a dataset and compare ADC vs exact distances.
...
def run_opq_eval(
    ...
    print(f"Training OPQ with {n_subquantizers}x{nbits} => {n_subquantizers * nbits} bits/vector")

    start = time.time()
    index.train(train_db)
    ...
    csv_path = Path(data_root) / results_dir / f"{dataset_name}_OPQ_adc_vs_exact_eval.csv"
    summary = {
        "method": "OPQ",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run OPQ ADC vs exact evaluation (dataset-agnostic).")
    ...
    parser.add_argument("--n_subquantizers", type=int, default=8)
    parser.add_argument("--nbits", type=int, default=9)
