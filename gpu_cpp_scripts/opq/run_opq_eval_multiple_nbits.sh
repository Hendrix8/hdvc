#!/usr/bin/env bash
# Wrapper script to run run_opq_eval_parallel_mult.sh sequentially for different datasets and nbits values
#
# Usage:
#   ./run_opq_eval_multiple_nbits.sh
#
...
#
# For each dataset, it automatically uses the appropriate M_VALUES for OPQ
...
# Optional: Override GPU devices (default: use what's in run_opq_eval_parallel_mult.sh)
GPU_DEVICES="0"

# Optional: Override datasets or nbits via command line
# Usage: ./run_opq_eval_multiple_nbits.sh [dataset1 dataset2 ...] [--nbits 4 6 8]
TEMP_DATASETS=()
TEMP_NBITS=()
PARSE_NBITS=false
...
echo "=========================================="
echo "Running OPQ experiments for multiple datasets and nbits"
echo "=========================================="
...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    for NBITS in "${NBITS_VALUES[@]}"; do
        ...
        # Export environment variables for run_opq_eval_parallel_mult.sh
        export DATASET_NAME
        export NBITS
        export M_VALUES_ENV
        ...
        if ./run_opq_eval_parallel_mult.sh; then
            echo ""
...
echo ""
echo "=========================================="
echo "All OPQ experiments completed!"
echo "=========================================="
