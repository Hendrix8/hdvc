// train_opq.cpp
// Use pre-learned OPQ rotations (from learn_opq_rot) to train PQ on GPU,
// run ADC evaluation, and write summary CSVs. This binary NEVER trains OPQ;
// it only loads an existing VectorTransform from --opq_model_path and
// reads its training time from the sidecar .meta file if present.

#include <faiss/VectorTransform.h>
#include <faiss/index_io.h>
#include <faiss/impl/ProductQuantizer.h>
#include <faiss/gpu/StandardGpuResources.h>
#include <faiss/gpu/GpuIndexFlat.h>

#include "../io_utils.h"

#include <cuda_runtime.h>

#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstring>
#include <filesystem>
#include <fstream>
#include <iomanip>
#include <iostream>
#include <limits>
#include <sstream>
#include <string>
#include <vector>

namespace fs = std::filesystem;

std::vector<float> flatten_vectors(const std::vector<std::vector<float>>& vecs) {
    if (vecs.empty()) {
        return {};
    }
    size_t dim = vecs[0].size();
    std::vector<float> flat(vecs.size() * dim);
    for (size_t i = 0; i < vecs.size(); i++) {
        std::memcpy(flat.data() + i * dim, vecs[i].data(), dim * sizeof(float));
    }
    return flat;
}

void compute_exact_distances(
        const float* queries,
        size_t nq,
        const float* database,
        size_t nb,
        size_t dim,
        float* distances) {
    for (size_t i = 0; i < nq; i++) {
        const float* q = queries + i * dim;
        for (size_t j = 0; j < nb; j++) {
            const float* d = database + j * dim;
            float dist = 0.0f;
            for (size_t k = 0; k < dim; k++) {
                float diff = q[k] - d[k];
                dist += diff * diff;
            }
            const float max_safe = std::numeric_limits<float>::max() / 10.0f;
            distances[i * nb + j] = std::min(dist, max_safe);
        }
    }
}

void compute_adc_distances(
        const float* dis_tables,
        const uint8_t* codes,
        size_t nq,
        size_t nb,
        size_t M,
        size_t ksub,
        size_t code_size,
        size_t nbits,
        float* adc_distances) {
    for (size_t i = 0; i < nq; i++) {
        const float* table = dis_tables + i * M * ksub;
        for (size_t j = 0; j < nb; j++) {
            const uint8_t* code = codes + j * code_size;
            float dist = 0.0f;

            if (nbits == 8) {
                for (size_t m = 0; m < M; m++) {
                    uint8_t idx = code[m];
                    dist += table[m * ksub + idx];
                }
            } else {
                faiss::PQDecoderGeneric decoder(code, nbits);
                for (size_t m = 0; m < M; m++) {
                    uint64_t idx = decoder.decode();
                    dist += table[m * ksub + idx];
                }
            }

            const float max_safe = std::numeric_limits<float>::max() / 10.0f;
            adc_distances[i * nb + j] = std::min(dist, max_safe);
        }
    }
}

void compute_relative_error(
        const float* adc_dist,
        const float* exact_dist,
        size_t nq,
        size_t nb,
        float& mean_rel,
        float& std_rel) {
    std::vector<double> rel_errors;
    const double epsilon = 1e-6;

    for (size_t i = 0; i < nq * nb; i++) {
        double exact = static_cast<double>(exact_dist[i]);
        double adc = static_cast<double>(adc_dist[i]);
        double denominator = std::max(exact, epsilon);
        double diff = std::abs(adc - exact);
        double rel_err = diff / denominator;

        if (std::isfinite(rel_err) && rel_err >= 0 && rel_err < 1e6) {
            rel_errors.push_back(rel_err);
        }
    }

    if (rel_errors.empty()) {
        mean_rel = std::nan("");
        std_rel = std::nan("");
        return;
    }

    double sum = 0.0;
    for (double err : rel_errors) {
        sum += err;
    }
    mean_rel = static_cast<float>(sum / rel_errors.size());

    double sq_sum = 0.0;
    for (double err : rel_errors) {
        double diff = err - mean_rel;
        sq_sum += diff * diff;
    }
    std_rel = static_cast<float>(
            std::sqrt(sq_sum / std::max<size_t>(1, rel_errors.size() - 1)));
}

void save_summary_txt(
        const std::string& txt_path,
        const std::string& dataset_name,
        const std::string& experiment_folder,
        size_t nq,
        size_t nb,
        size_t nb_sample,
        size_t dim,
        size_t M,
        size_t nbits,
        size_t train_size,
        size_t rot_train_size,
        size_t opq_max_train_points,
        double train_time,
        double opq_train_time,
        double pq_train_time,
        double encoding_time,
        double distance_table_time,
        double cdist_time,
        double adc_time,
        float mean_rel,
        float std_rel) {
    std::ofstream file(txt_path);
    if (!file) {
        std::cerr << "Warning: Failed to open " << txt_path << " for writing\n";
        return;
    }

    file << "method: OPQ\n";
    file << "dataset: " << dataset_name << "\n";
    file << "experiment_folder: " << experiment_folder << "\n";
    file << "nq: " << nq << "\n";
    file << "nb: " << nb << "\n";
    file << "nb_sample: " << nb_sample << "\n";
    file << "dim: " << dim << "\n";
    file << "n_subquantizers: " << M << "\n";
    file << "nbits: " << nbits << "\n";
    file << "bits_per_vector: " << (M * nbits) << "\n";
    file << "train_size: " << train_size << "\n";
    file << "rot_train_sz: " << rot_train_size << "\n";
    file << "max_opq_rot_train_samples: " << opq_max_train_points << "\n";
    file << std::fixed << std::setprecision(6);
    file << "train_time_s: " << train_time << "\n";
    file << "opq_train_time_s: " << opq_train_time << "\n";
    file << "pq_train_time_s: " << pq_train_time << "\n";
    file << "encoding_time_s: " << encoding_time << "\n";
    file << "distance_table_time_s: " << distance_table_time << "\n";
    file << "cdist_time_s: " << cdist_time << "\n";
    file << "adc_time_s: " << adc_time << "\n";
    file << "rel_error_mean: " << (std::isnan(mean_rel) ? 0.0 : mean_rel) << "\n";
    file << "rel_error_std: " << (std::isnan(std_rel) ? 0.0 : std_rel) << "\n";
    file.close();
}

void save_summary_csv(
        const std::string& csv_path,
        const std::string& dataset_name,
        const std::string& experiment_folder,
        size_t nq,
        size_t nb,
        size_t nb_sample,
        size_t dim,
        size_t M,
        size_t nbits,
        size_t train_size,
        size_t rot_train_size,
        size_t opq_max_train_points,
        double train_time,
        double opq_train_time,
        double pq_train_time,
        double encoding_time,
        double distance_table_time,
        double cdist_time,
        double adc_time,
        float mean_rel,
        float std_rel) {
    bool need_header = !fs::exists(csv_path) || fs::file_size(csv_path) == 0;

    std::ofstream file(csv_path, std::ios::app);
    if (!file) {
        std::cerr << "Warning: Failed to open " << csv_path << " for appending\n";
        return;
    }

    if (need_header) {
        file << "method,dataset,experiment_folder,nq,nb,nb_sample,dim,n_subquantizers,nbits,"
             << "bits_per_vector,train_size,rot_train_sz,max_opq_rot_train_samples,"
             << "train_time_s,opq_train_time_s,pq_train_time_s,"
             << "encoding_time_s,distance_table_time_s,cdist_time_s,adc_time_s,"
             << "rel_error_mean,rel_error_std\n";
    }

    file << "OPQ," << dataset_name << "," << experiment_folder << ","
         << nq << "," << nb << "," << nb_sample << "," << dim << ","
         << M << "," << nbits << "," << (M * nbits) << ","
         << train_size << "," << rot_train_size << "," << opq_max_train_points << ","
         << std::fixed << std::setprecision(6)
         << train_time << "," << opq_train_time << "," << pq_train_time << ","
         << encoding_time << "," << distance_table_time << "," << cdist_time << ","
         << adc_time << ",";

    if (std::isnan(mean_rel)) {
        file << "nan,";
    } else {
        file << mean_rel << ",";
    }

    if (std::isnan(std_rel)) {
        file << "nan\n";
    } else {
        file << std_rel << "\n";
    }

    file.close();
}

// Read OPQ metadata from the sidecar .meta file if available.
// Format (whitespace-separated, single line):
//   opq_train_time_s rot_train_size opq_max_train_points
// Older files may contain only opq_train_time_s; in that case the
// additional fields are left as 0.
void load_opq_meta(
        const std::string& opq_model_path,
        double& opq_train_time,
        size_t& rot_train_size,
        size_t& opq_max_train_points) {
    opq_train_time = 0.0;
    rot_train_size = 0;
    opq_max_train_points = 0;
    fs::path meta_path = fs::path(opq_model_path).concat(".meta");
    if (!fs::exists(meta_path)) {
        std::cerr << "Warning: OPQ meta file not found: " << meta_path
                  << " – using default OPQ meta values\n";
        return;
    }

    std::ifstream meta(meta_path);
    if (!meta) {
        std::cerr << "Warning: Failed to open OPQ meta file: " << meta_path
                  << " – using default OPQ meta values\n";
        return;
    }

    // Try to parse up to three values; missing ones stay at 0.
    meta >> opq_train_time >> rot_train_size >> opq_max_train_points;
    if (!meta.good()) {
        std::cerr << "Warning: OPQ meta file parsed partially or incompletely: " << meta_path
                  << " – opq_train_time_s=" << opq_train_time
                  << ", rot_train_size=" << rot_train_size
                  << ", opq_max_train_points=" << opq_max_train_points << std::endl;
        return;
    }
    std::cout << "📖 Loaded OPQ meta (" << meta_path
              << "): train_time_s=" << opq_train_time
              << ", rot_train_size=" << rot_train_size
              << ", max_train_pts=" << opq_max_train_points << std::endl;
}

int main(int argc, char* argv[]) {
    if (argc < 7) {
        std::cerr << "Usage: " << argv[0]
                  << " --dataset_path PATH --query_path PATH --train_path PATH "
                  << "[--dim DIM] [--dataset_name NAME] [--data_root ROOT] "
                  << "[--n_subquantizers M] [--nbits BITS] [--train_size SIZE] "
                  << "[--sample_db SIZE] [--sample_queries SIZE] [--results_dir DIR] "
                  << "[--opq_model_path PATH] [--gpu_device DEVICE]\n";
        return 1;
    }

    std::string dataset_path, query_path, train_path;
    int dim = -1;
    std::string dataset_name = "custom";
    std::string data_root = "/data/cpanourg/2-hdvc/";
    size_t M = 32;
    size_t nbits = 8;
    size_t train_size = 100000;
    size_t sample_db = 10000;
    size_t sample_queries = 1000;
    std::string results_dir = "results/relerr_cpp";
    std::string opq_model_path;  // required for this binary
    int gpu_device = 0;

    for (int i = 1; i < argc; i++) {
        std::string arg = argv[i];
        if (arg == "--dataset_path" && i + 1 < argc) {
            dataset_path = argv[++i];
        } else if (arg == "--query_path" && i + 1 < argc) {
            query_path = argv[++i];
        } else if (arg == "--train_path" && i + 1 < argc) {
            train_path = argv[++i];
        } else if (arg == "--dim" && i + 1 < argc) {
            dim = std::stoi(argv[++i]);
        } else if (arg == "--dataset_name" && i + 1 < argc) {
            dataset_name = argv[++i];
        } else if (arg == "--data_root" && i + 1 < argc) {
            data_root = argv[++i];
        } else if (arg == "--n_subquantizers" && i + 1 < argc) {
            M = std::stoul(argv[++i]);
        } else if (arg == "--nbits" && i + 1 < argc) {
            nbits = std::stoul(argv[++i]);
        } else if (arg == "--train_size" && i + 1 < argc) {
            train_size = std::stoul(argv[++i]);
        } else if (arg == "--sample_db" && i + 1 < argc) {
            sample_db = std::stoul(argv[++i]);
        } else if (arg == "--sample_queries" && i + 1 < argc) {
            sample_queries = std::stoul(argv[++i]);
        } else if (arg == "--results_dir" && i + 1 < argc) {
            results_dir = argv[++i];
        } else if (arg == "--opq_model_path" && i + 1 < argc) {
            opq_model_path = argv[++i];
        } else if (arg == "--gpu_device" && i + 1 < argc) {
            gpu_device = std::stoi(argv[++i]);
        }
    }

    if (dataset_path.empty() || query_path.empty() || train_path.empty()) {
        std::cerr << "Error: dataset_path, query_path, and train_path are required\n";
        return 1;
    }

    if (opq_model_path.empty()) {
        std::cerr << "Error: --opq_model_path is required (pre-trained OPQ transform)\n";
        return 1;
    }

    if (!fs::exists(opq_model_path)) {
        std::cerr << "Error: OPQ model file not found at " << opq_model_path << "\n";
        return 1;
    }

    std::cout << "📂 Loading database from " << dataset_path << std::endl;
    auto db_vecs = load_dataset(dataset_path, dim, 1000000);
    if (db_vecs.empty()) {
        std::cerr << "Error: Failed to load database\n";
        return 1;
    }
    std::cout << "Loaded database: " << db_vecs.size() << " vectors" << std::endl;

    std::cout << "📂 Loading training data from " << train_path << std::endl;
    auto train_db_full = load_dataset(train_path, dim, train_size + 10000);
    if (train_db_full.empty()) {
        std::cerr << "Error: Failed to load training data\n";
        return 1;
    }
    std::cout << "Loaded training data: " << train_db_full.size() << " vectors" << std::endl;

    std::cout << "📂 Loading queries from " << query_path << std::endl;
    auto qr_vecs = load_dataset(query_path, dim, sample_queries);
    if (qr_vecs.empty()) {
        std::cerr << "Error: Failed to load queries\n";
        return 1;
    }
    std::cout << "Loaded queries: " << qr_vecs.size() << " vectors" << std::endl;

    size_t actual_dim = db_vecs[0].size();
    if (dim > 0 && static_cast<size_t>(dim) != actual_dim) {
        std::cerr << "Warning: Dimension mismatch. Expected " << dim
                  << ", got " << actual_dim << std::endl;
    }
    dim = static_cast<int>(actual_dim);

    if (dim % static_cast<int>(M) != 0) {
        std::cerr << "Error: dim must be divisible by n_subquantizers (M)\n";
        return 1;
    }

    size_t actual_train_size = std::min(train_size, train_db_full.size());
    std::vector<std::vector<float>> train_db(
            train_db_full.begin(),
            train_db_full.begin() + actual_train_size);

    size_t test_size = std::min(1000000UL, db_vecs.size());
    std::vector<std::vector<float>> test_db(
            db_vecs.begin(),
            db_vecs.begin() + test_size);

    size_t nb = test_db.size();
    size_t nq = qr_vecs.size();

    std::cout << "Training PQ on " << train_db.size() << " samples, "
              << "testing on " << nb << " samples, "
              << "queries: " << nq << ", dim=" << dim << std::endl;

    auto start = std::chrono::high_resolution_clock::now();
    auto end = start;

    // Load OPQ transform (no training here) and associated metadata.
    std::cout << "📥 Loading OPQ transform from " << opq_model_path << std::endl;
    std::unique_ptr<faiss::VectorTransform> opq_loaded(
            faiss::read_VectorTransform(opq_model_path.c_str()));
    if (!opq_loaded) {
        std::cerr << "Error: Failed to read OPQ transform" << std::endl;
        return 1;
    }
    if (opq_loaded->d_in != dim || opq_loaded->d_out != dim) {
        std::cerr << "Error: Loaded OPQ transform dimensions don't match current dim"
                  << std::endl;
        return 1;
    }
    faiss::VectorTransform* opq_vt = opq_loaded.get();

    double opq_train_time = 0.0;
    size_t opq_rot_train_size = 0;
    size_t opq_max_train_points = 0;
    load_opq_meta(opq_model_path, opq_train_time, opq_rot_train_size, opq_max_train_points);

    auto train_flat = flatten_vectors(train_db);
    std::vector<float> train_rot(train_db.size() * dim);
    opq_vt->apply_noalloc(train_db.size(), train_flat.data(), train_rot.data());

    faiss::ProductQuantizer pq(dim, M, nbits);
    std::cout << "Training PQ with " << M << "x" << nbits
              << " => " << (M * nbits) << " bits/vector (GPU)\n";

    faiss::gpu::StandardGpuResources gpu_res;
    faiss::gpu::GpuIndexFlatConfig flat_config;
    flat_config.device = gpu_device;
    faiss::gpu::GpuIndexFlatL2 gpu_assign_index(&gpu_res, pq.dsub, flat_config);
    pq.assign_index = &gpu_assign_index;

    auto pq_start = std::chrono::high_resolution_clock::now();
    pq.train(train_db.size(), train_rot.data());
    cudaDeviceSynchronize();
    auto pq_end = std::chrono::high_resolution_clock::now();
    double pq_train_time =
            std::chrono::duration<double>(pq_end - pq_start).count();

    double train_time = opq_train_time + pq_train_time;

    pq.assign_index = nullptr;
    std::cout << "✅ PQ trained (with pre-learned OPQ) in " << train_time << "s"
              << " [OPQ=" << opq_train_time << "s, PQ=" << pq_train_time << "s]" << std::endl;

    std::cout << "Encoding database..." << std::endl;
    start = std::chrono::high_resolution_clock::now();

    auto test_flat = flatten_vectors(test_db);
    std::vector<float> test_rot(nb * dim);
    opq_vt->apply_noalloc(nb, test_flat.data(), test_rot.data());

    size_t code_size = pq.code_size;
    std::vector<uint8_t> codes(nb * code_size);
    pq.compute_codes(test_rot.data(), codes.data(), nb);

    end = std::chrono::high_resolution_clock::now();
    double encoding_time = std::chrono::duration<double>(end - start).count();
    std::cout << "✅ Encoding done in " << encoding_time << "s | "
              << "Codes shape: " << nb << " x " << code_size << std::endl;

    std::cout << "Computing distance tables..." << std::endl;
    start = std::chrono::high_resolution_clock::now();

    size_t ksub = 1 << nbits;
    auto query_flat = flatten_vectors(qr_vecs);
    std::vector<float> query_rot(nq * dim);
    opq_vt->apply_noalloc(nq, query_flat.data(), query_rot.data());

    std::vector<float> dis_tables(nq * M * ksub);
    pq.compute_distance_tables(nq, query_rot.data(), dis_tables.data());

    end = std::chrono::high_resolution_clock::now();
    double distance_table_time =
            std::chrono::duration<double>(end - start).count();
    std::cout << "✅ Distance tables computed in " << distance_table_time << "s" << std::endl;

    size_t n_sample_q = std::min(sample_queries, nq);
    size_t n_sample_db = std::min(sample_db, nb);

    std::vector<float> qr_sample(n_sample_q * dim);
    std::vector<float> db_sample(n_sample_db * dim);

    for (size_t i = 0; i < n_sample_q; i++) {
        std::memcpy(
                qr_sample.data() + i * dim,
                qr_vecs[i].data(),
                dim * sizeof(float));
    }
    for (size_t i = 0; i < n_sample_db; i++) {
        std::memcpy(
                db_sample.data() + i * dim,
                test_db[i].data(),
                dim * sizeof(float));
    }

    std::vector<float> adc_sample(n_sample_q * n_sample_db);
    std::vector<uint8_t> codes_sample(n_sample_db * code_size);
    for (size_t i = 0; i < n_sample_db; i++) {
        std::memcpy(
                codes_sample.data() + i * code_size,
                codes.data() + i * code_size,
                code_size);
    }

    std::vector<float> dis_tables_sample(n_sample_q * M * ksub);
    for (size_t i = 0; i < n_sample_q; i++) {
        std::memcpy(
                dis_tables_sample.data() + i * M * ksub,
                dis_tables.data() + i * M * ksub,
                M * ksub * sizeof(float));
    }

    std::cout << "Computing ADC distances..." << std::endl;
    start = std::chrono::high_resolution_clock::now();
    compute_adc_distances(
            dis_tables_sample.data(),
            codes_sample.data(),
            n_sample_q,
            n_sample_db,
            M,
            ksub,
            code_size,
            nbits,
            adc_sample.data());
    end = std::chrono::high_resolution_clock::now();
    double adc_time = std::chrono::duration<double>(end - start).count();
    std::cout << "✅ ADC distances computed in " << adc_time << "s" << std::endl;

    std::cout << "Computing exact distances..." << std::endl;
    start = std::chrono::high_resolution_clock::now();
    std::vector<float> exact_sample(n_sample_q * n_sample_db);
    compute_exact_distances(
            qr_sample.data(),
            n_sample_q,
            db_sample.data(),
            n_sample_db,
            dim,
            exact_sample.data());
    end = std::chrono::high_resolution_clock::now();
    double cdist_time = std::chrono::duration<double>(end - start).count();
    std::cout << "✅ Exact distances computed in " << cdist_time << "s" << std::endl;

    float mean_rel, std_rel;
    compute_relative_error(
            adc_sample.data(),
            exact_sample.data(),
            n_sample_q,
            n_sample_db,
            mean_rel,
            std_rel);

    std::cout << "Mean rel. error: " << mean_rel << ", std: " << std_rel << std::endl;

    auto now = std::chrono::system_clock::now();
    auto time_t = std::chrono::system_clock::to_time_t(now);
    std::stringstream ss;
    ss << std::put_time(std::localtime(&time_t), "%Y%m%d_%H%M%S");
    std::string timestamp = ss.str();

    fs::path out_root(results_dir);
    fs::path out_dir =
            out_root / dataset_name / ("M" + std::to_string(M)) /
            ("nbits" + std::to_string(nbits));
    fs::create_directories(out_dir);

    std::string txt_path = (out_dir / ("summary_" + timestamp + ".txt")).string();
    // Per‑dataset aggregate CSV collecting all (M, nbits) runs for this method.
    std::string csv_path =
            (out_root / (dataset_name + "_OPQ_adc_vs_exact_eval.csv")).string();

    save_summary_txt(
            txt_path,
            dataset_name,
            out_dir.string(),
            nq,
            nb,
            n_sample_db,
            dim,
            M,
            nbits,
            actual_train_size,
            opq_rot_train_size,
            opq_max_train_points,
            train_time,
            opq_train_time,
            pq_train_time,
            encoding_time,
            distance_table_time,
            cdist_time,
            adc_time,
            mean_rel,
            std_rel);

    std::string run_csv = (out_dir / "summary.csv").string();
    save_summary_csv(
            run_csv,
            dataset_name,
            out_dir.string(),
            nq,
            nb,
            n_sample_db,
            dim,
            M,
            nbits,
            actual_train_size,
            opq_rot_train_size,
            opq_max_train_points,
            train_time,
            opq_train_time,
            pq_train_time,
            encoding_time,
            distance_table_time,
            cdist_time,
            adc_time,
            mean_rel,
            std_rel);

    save_summary_csv(
            csv_path,
            dataset_name,
            out_dir.string(),
            nq,
            nb,
            n_sample_db,
            dim,
            M,
            nbits,
            actual_train_size,
            opq_rot_train_size,
            opq_max_train_points,
            train_time,
            opq_train_time,
            pq_train_time,
            encoding_time,
            distance_table_time,
            cdist_time,
            adc_time,
            mean_rel,
            std_rel);

    std::cout << "✅ Results saved to " << out_dir << std::endl;
    return 0;
}

