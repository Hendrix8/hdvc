...
RUN_ROOT="${RUN_ROOT:-${RESULTS_ROOT}/saq_native_ivf_nohup_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${RUN_ROOT}"
...
if [[ "${APPEND_LOG}" == "1" ]]; then
  nohup "${AFFINITY[@]}" "${PYTHON}" -u -m saq.saq_ivf_grid --output_root "${RUN_ROOT}" "$@" >> "${LOG}" 2>&1 &
else
  nohup "${AFFINITY[@]}" "${PYTHON}" -u -m saq.saq_ivf_grid --output_root "${RUN_ROOT}" "$@" > "${LOG}" 2>&1 &
fi
echo $! > "${PID_FILE}"

echo "Launched SAQ IVF grid"
