...
def shuffled_hnsw_to_standard_form(graph, new2old, M):
    ...
    for i in range(len(graph)):
        if (i % 100000 == 0):
            print(i)
        ret_graph[new2old[i]] = np.pad(np.array([new2old[x] for x in graph[i]]), (0, M + M - len(graph[i])),
                                       constant_values=-1)
...
# Function to append rows or create file if it doesn't exist
def append_or_create_csv(file_name, header, rows):
