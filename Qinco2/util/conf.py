...
class Conf:
    def __init__(self, conf_filepath: str = None, query_only: bool = False) -> None:
        ...
        self.defaults = {
            ...
            'train_positive_labels': None,
            'train_negative_labels': None,
            'valid_positive_labels': None,
            'valid_negative_labels': None,
            ...
            'transformer_t_norm': 'BatchNorm',
            'transformer_freeze': False,

            'seasam_n_segment': 32,
            'seasam_max_cardinality': 4
        }

        self.legals = {
            'device': {'cpu', 'cuda'},

            'model': {'resnet', 'resnetfr', 'res1d18', 'incept', 'rnn', 'transformer'},
            'rnn_type': {'lstm', 'gru'},
            ...
            'num_samples_method': {'none', 'balanced', 'cvpr19', 'fixed'},
            'sample_method': {'random', 'latent', 'seasam'},
            ...
            'transformer_pos_encoding': {'fixed', 'learnable'},
            'transformer_activation': {'relu', 'gelu'},
            'transformer_t_norm': {'BatchNorm', 'LayerNorm'},
        }
        ...
        for key, value in config.items():
            if key not in self.useless_keys:
                if key not in self.legals or value in self.legals[key]:
                    ...
                    raise ValueError(
                        'invalid hyperparameter {:s}: {:s}'.format(key, str(value)))

    def setup(self):
        # assert type(self.getHP('train_positive_samples')) is np.ndarray and type(self.getHP('train_negative_samples')) is np.ndarray
        # assert type(self.getHP('valid_positive_samples')) is np.ndarray and type(self.getHP('valid_negative_samples')) is np.ndarray
        ...
        assert not (self.getHP('model') == 'resnetfr' and self.getHP(
            'loss_function') == 'sasu')
        ...
        if self.getHP('checkpoint_filepath') is None:
            assert self.getHP('checkpoint_folderpath') is not None and type(
                self.getHP('checkpoint_folderpath')) is str

            self.setHP('checkpoint_filepath', os.path.join(self.getHP(
                'checkpoint_folderpath'), self.getHP('checkpoint_filename')))

        if self.getHP('imbalance_expanding') != 'none':
            assert 1 < self.getHP('increxp_epochbase') <= self.getHP(
                'increxp_numnegbase')

        if self.getHP('loss_function') == 'focal':
            assert not self.getHP('focal')

        if self.getHP('num_samples_method') == 'fixed':
            assert self.getHP('num_training_negative_samples') > 0
    ...
    def dumpConf(self, filepath: str = None) -> None:
        ...
        with open(filepath, 'w') as fout:
            ...
            for key2check in ['train_positive_samples', 'train_negative_samples', 'valid_positive_samples', 'valid_negative_samples']:
                if self.settings_copy[key2check] is not None and self.settings_copy[key2check] is not str:
                    self.settings_copy[key2check] = str(
                        self.settings_copy[key2check].shape)

            json.dump({'settings': self.settings_copy,
                      'defaults': self.defaults}, fout, sort_keys=True, indent=4)

    def getCheckpointFilepath(self, epoch: int) -> str:
        if epoch < 0:
            return None

        return os.path.join(self.getHP('checkpoint_folderpath'), str(epoch) + self.getHP('checkpoint_filepostfix'))
