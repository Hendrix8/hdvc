                    ...
                    if os.path.isdir(setting_folderpath):
                        for tune in os.listdir(setting_folderpath):
                            ...
                            if os.path.isdir(tune_folderpath):
                                if set(['train.log']).issubset(os.listdir(tune_folderpath)) and checkF(tune_folderpath):
                                    setting_segments = setting.split('-')
                                    beta_fetched = float(setting_segments[5])
                                    ...
                                    model_filepath = os.path.join(tune_folderpath, model_filename)
                                    
                                    if not os.path.exists(model_filepath):
                                        epoch = 100
                                        checkpoint_filename = str(epoch) + '.pickle'
                                        checkpoint_filepath = os.path.join(tune_folderpath, checkpoint_filename)

                                        while epoch >= 0 and checkpoint_filepath is not None and not os.path.exists(checkpoint_filepath):
                                            epoch -= 1
                                            checkpoint_filename = str(epoch) + '.pickle'
                                            checkpoint_filepath = os.path.join(tune_folderpath, checkpoint_filename)
                                    ...
                                    if finished:
                                        ...
                                        if len(results2sortby) > 0 and not np.isnan(results2sortby[-1][positions2compare[1]]):
                                            ...
                                            if self.fetch_all:
                                                self.results[patient_id][setting].append([valid_results[-1], train_results[-1], model_filepath])
                                                    ...
                                                    self.results[patient_id][setting] = [valid_results[-1], train_results[-1], model_filepath]
