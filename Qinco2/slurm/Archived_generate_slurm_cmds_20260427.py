...
def generate_cmds(args, dataset, M, K):
    ...
    if args.mode == 'train':
        ...
        cd_cmd = f'\ncd {save_path}'
        main_cmd = (f'python $VQ_CODE/slurm/submit_job_Qinco.py '    # 注意：不要删掉结尾空格！
               f'"$VQ_CODE/run.sh '
               f'task={args.mode} '
    ...
    if args.mode in ('encode_train', 'encode_test', 'encode_queries'):
        ...
        save_path, i_setting = get_setting_full_path(conf, main_save_path)
    ...
    if args.mode == 'train_qinco_aq':
        ...
        encode_train_save_path, encode_train_i_setting = get_setting_full_path(
            encode_train_conf, os.path.join(train_save_path, 'encode_train'))
        ...
        save_path, i_setting = get_setting_full_path(conf, tqa_main_save_path)
        ...
        if run_option == 'from_scratch':
            ...
            cd_cmd = f'\ncd {save_path}'
            main_cmd = (f'python $VQ_CODE/run.py '
    ...
    if args.mode == 'qinco_aq_compute_distances':
        ...
        DirProcessor.create_dir(tqa_main_save_path)
        tqa_save_path, tqa_i_setting = get_setting_full_path(conf, tqa_main_save_path)
        ...
        encode_test_save_path, encode_test_i_setting = get_setting_full_path(encode_test_conf,
    ...
    if args.mode == 'train_pairwise_decoder':
        ...
        save_path, i_setting = get_setting_full_path(conf, tpd_main_save_path)
    ...
    if args.mode == 'shen_build_indices':
        ...
        tpd_main_save_path = os.path.join(encode_train_save_path, 'train_pairwise_decoder')
        DirProcessor.create_dir(tpd_main_save_path)
        tpd_save_path, tpd_i_setting = get_setting_full_path(conf, tpd_main_save_path)
        ...
        encode_test_main_save_path = os.path.join(train_save_path, 'encode_test')
        encode_test_save_path, encode_test_i_setting = get_setting_full_path(encode_test_conf, encode_test_main_save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_db', type=int, default=None)
    ...
    parser.add_argument('--tqa_n_train', type=int, default=None)
    parser.add_argument('--tqa_n_val', type=int, default=None)

    # parameters for 'train_pairwise_decoder'
    parser.add_argument('--tpd_n_train', type=int, default=None)
    parser.add_argument('--tpd_n_val', type=int, default=None)
    ...
    parser.add_argument('--slurm_time', default='10:00:00') # 注意：要故意把slurm_time设置得比max_sec_total更长一些
    ...
    all_M_by_dataset = {
        dataset: args.all_M if args.all_M is not None else default_data_info[dataset]['all_M']
